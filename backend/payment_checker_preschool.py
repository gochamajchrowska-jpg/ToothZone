# -*- coding: utf-8 -*-
"""
payment_checker_preschool.py
Pobiera platnosci dla Majchrowska-Zab Iga z oplaty@cui.wroclaw.pl.
Zapisuje do preschool_payment_messages.json.
"""
...
def is_for_iga(body):
    """Czy mail dotyczy Majchrowska-Zab Iga?"""
...
def fetch_and_deduplicate():
    ...
    try:
        ...
        for mail_id in mail_ids:
            ...
            if not is_for_iga(body):
                print(f"[SKIP] {mail_id.decode()} — nie dotyczy Igi", file=sys.stderr)
                continue
            ...
            if not key:
                print(f"[SKIP] {mail_id.decode()} — brak miesiaca", file=sys.stderr)
                continue

            if key in unique:
                print(f"[SKIP] Duplikat '{key}'", file=sys.stderr)
                continue
